...
import { listMemorizedPage, unmemorize} from '../services/db';

const PAGE_SIZE = 20;

export default function MemorizedGemsPage() {
    ...
    useEffect(() => {
        (async () => {
            const { rows, total } = await listMemorizedPage(page, PAGE_SIZE);
            setRows(rows);
            setTotal(total);
            setSel({});
        })();
    ...
    async function handleUnmemorize() {
        if (!selectedIds.length) return;
        setBusy(true);
        try {
            await unmemorize(selectedIds);
            const { rows: r2, total: t2 } = await listMemorizedPage(page, PAGE_SIZE);
            if (r2.length === 0 && page > 0) {
                const prevPage = page - 1;
                const { rows: r3, total: t3 } = await listMemorizedPage(prevPage, PAGE_SIZE);
                setPage(prevPage);
                setRows(r3);
                setTotal(t3);
            } else {
    ...
    return (
        <section className="py-4">
            <header className="mb-3">
                <h1 className="text-xl font-semibold">Gems I’ve memorized</h1>
                <p className="text-slate-400 text-sm">
                    These won’t show in practice until their review window is over.
                </p>
            </header>
            ...
            <div className="overflow-x-auto rounded-xl border border-slate-800">
                <table className="w-full text-sm">
                    ...
                    <tr>
                        ...
                        <th className="px-3 py-2 text-left text-slate-400">Memorized at</th>
                        <th className="px-3 py-2 text-left text-slate-400">Review after</th>
                    </tr>
                    </thead>
                    <tbody>
                    {rows.map(r => (
                        <tr key={r.id} className="border-t border-slate-800 align-top">
                            ...
                            <td className="px-3 py-2 text-slate-400">
                                {new Date(r.memorizedAt).toLocaleDateString()}
                            </td>
                            <td className="px-3 py-2 text-slate-400">{r.reviewAfter}</td>
                        </tr>
                    ))}
                    {rows.length === 0 && (
                        <tr>
